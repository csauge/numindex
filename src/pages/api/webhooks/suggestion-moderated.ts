...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  try {
    ...
    const subject = `[numindex.org] Votre suggestion a été ${statusLabel}`;
    ...
    const htmlContent = `
      <div style="font-family: sans-serif; line-height: 1.5; color: #333; max-width: 600px; margin: 0 auto;">
        ...
        <p>Bonjour,</p>
        <p>Nous vous informons que votre suggestion pour la ressource <strong>"${record.title}"</strong> a été <strong>${statusLabel}</strong> par notre équipe de modération.</p>
        
        ${record.status === 'approved' 
          ? `<p>Elle est désormais visible sur le site ! Merci pour votre contribution à un numérique plus responsable.</p>
